...
inline std::string vformat_string(const char *format, va_list args){
	va_list args_copy;
...
inline double logt(const char *format, struct timeval &start, ...){
	...
	va_list args;
	va_start(args, start);
	std::string sprint_buf = vformat_string(format, args);
	va_end(args);
	long tid = 0;
#if __linux
	...
	fprintf(stderr,"%s thread %ld:\t%s", time_string().c_str(), tid, sprint_buf.c_str());
...
inline void log(const char *format, ...){
	...
	tid = GetCurrentThreadId();
#endif
	fprintf(stderr,"%s thread %ld:\t%s\n", time_string().c_str(), tid, sprint_buf.c_str());
	fflush(stderr);
	pthread_mutex_unlock(&print_lock);
}
