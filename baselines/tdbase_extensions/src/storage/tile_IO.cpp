...
void Tile::dump_raw(const char *path){
	...
	std::vector<char> buffer(std::max<size_t>(data_size * 20, 1024));
	size_t offset = 0;
	auto ensure_capacity = [&](size_t bytes) {
		if (offset + bytes > buffer.size()) {
			size_t next_size = std::max(buffer.size() * 2, offset + bytes);
	...
	for(HiMesh_Wrapper *wr:objects){
		...
		ensure_capacity(sizeof(size_t));
		size_t dsize_offset = offset;
		...
		for(int lod=20;lod<=100;lod+=20){
			...
			for(Voxel *v:wr->voxels){
				...
				size_t bytes = v->num_triangles*sizeof(float)*9;
				ensure_capacity(bytes);
				memcpy(buffer.data()+offset, v->triangles, bytes);
				offset += bytes;
				bytes = v->num_triangles*sizeof(float)*2;
				ensure_capacity(bytes);
				memcpy(buffer.data()+offset, v->hausdorff, bytes);
				offset += bytes;
			}
		}
		// update the data size
		ensure_capacity(sizeof(size_t));
		*(size_t *)(buffer.data()+dsize_offset) = offset-st_offset;

		// store the voxel number (put it here for aligning with the decoding mode)
		ensure_capacity(sizeof(size_t));
		*(size_t *)(buffer.data() + offset) = wr->voxels.size();
		offset += sizeof(size_t);

		// store the polyhedron-level hausdorff information for all the LODs
		for(int lod=20;lod<=100;lod+=20){
			ensure_capacity(sizeof(float) * 2);
			*(float *)(buffer.data() + offset) = hausdorffs[lod];
			offset += sizeof(float);
			*(float *)(buffer.data() + offset) = proxyhausdorffs[lod];
			offset += sizeof(float);
		}

		// store the voxel information
		for(Voxel *v:wr->voxels){
			ensure_capacity(sizeof(float)*9 + sizeof(size_t)*10);
			memcpy(buffer.data()+offset, v->low, sizeof(float)*3);
			offset += 3*sizeof(float);
			memcpy(buffer.data()+offset, v->high, sizeof(float)*3);
			offset += 3*sizeof(float);
			memcpy(buffer.data()+offset, v->core, sizeof(float)*3);
			offset += 3*sizeof(float);
			for(int lod=20;lod<=100;lod+=20){
				*(size_t *)(buffer.data()+offset) = v->offset_lod[lod];
				offset += sizeof(size_t);
				*(size_t *)(buffer.data()+offset) = v->volume_lod[lod];
				offset += sizeof(size_t);
			}
		}
	}

	os->write(buffer.data(), offset);
	os->close();
	delete os;
